...
| Export to standard output | `dopbase export <ENVIRONMENT_REF> --stdout`        | Defaults to dotenv. Use `--format json`, `--format yaml`, or export-only `--format docker` for another serialization                  | `dopbase export payment-service/staging --stdout --format docker`                         |
