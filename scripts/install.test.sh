...
printf '%s\n' '#!/bin/sh' 'printf "%s\\n" "dopbase 0.0.1"' >"${test_dir}/dopbase"
chmod 755 "${test_dir}/dopbase"

(cd "$test_dir" && zip -q "${release_dir}/dopbase_0.0.1_darwin_arm64.zip" dopbase)

if command -v sha256sum >/dev/null 2>&1; then
  checksum=$(sha256sum "${release_dir}/dopbase_0.0.1_darwin_arm64.zip" | awk '{ print $1 }')
else
  checksum=$(shasum -a 256 "${release_dir}/dopbase_0.0.1_darwin_arm64.zip" | awk '{ print $1 }')
fi
printf '%s  %s\n' "$checksum" "dopbase_0.0.1_darwin_arm64.zip" >"${release_dir}/checksums.txt"
...
test "$("${install_dir}/dopbase" --version)" = "dopbase 0.0.1"

printf '%064d  %s\n' 0 "dopbase_0.0.1_darwin_arm64.zip" >"${release_dir}/checksums.txt"
if PATH="${mock_bin}:${PATH}" \
  TEST_UNAME_S=Darwin \
  TEST_UNAME_M=arm64 \
  DOPBASE_VERSION=0.0.1 \
  DOPBASE_INSTALL_DIR="$install_dir" \
  DOPBASE_DOWNLOAD_BASE_URL="file://${release_dir}" \
...
if PATH="${mock_bin}:${PATH}" TEST_UNAME_S=Plan9 TEST_UNAME_M=arm64 \
  DOPBASE_VERSION=0.0.1 sh "${root_dir}/scripts/install.sh" >/dev/null 2>&1; then
