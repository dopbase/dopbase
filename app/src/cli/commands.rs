...
async fn connect(value: &str, data_dir: Option<&Path>, json_output: bool) -> Result<()> {
...
fn show_config(argument: Option<&str>, data_dir: Option<&Path>, json_output: bool) -> Result<()> {
...
async fn execute_client(
    ...
    match command {
        Command::Init {
            ...
            let data=api.request(Method::POST,"/api/v1/projects/init",Some(json!({"projectName":project,"environmentName":environment,"entries":entries}))).await?;
...
async fn project(
    ...
    let data = match command {
        ProjectCommand::Create { name } => {
            api.request(Method::POST, "/api/v1/projects", Some(json!({"name":name})))
                .await?
        }
        ProjectCommand::List => api.request(Method::GET, "/api/v1/projects", None).await?,
        ProjectCommand::Show { project } => {
            api.request(Method::GET, &format!("/api/v1/projects/{project}"), None)
                .await?
        }
        ...
        ProjectCommand::Delete { project, yes } => {
            ...
            api.request(Method::DELETE, &format!("/api/v1/projects/{project}"), None)
                .await?
        }
    };
    print_value(json_output, &data);
...
async fn resolve_environment(api: &ApiClient, reference: &str) -> Result<Value> {
    api.request(
        Method::GET,
        &format!(
...
async fn environment(
    ...
    let data = match command {
        ...
        EnvCommand::Show { environment } => {
            ...
            let id = env_id(&env)?;
            api.request(Method::GET, &format!("/api/v1/environments/{id}"), None)
                .await?
        }
        EnvCommand::Rename {
            environment,
        ...
        EnvCommand::Delete { environment, yes } => {
            ...
            api.request(Method::DELETE, &format!("/api/v1/environments/{id}"), None)
                .await?
        }
    };
    print_value(json_output, &data);
...
async fn admin(command: AdminCommand, data_dir: Option<PathBuf>) -> Result<i32> {
    match command {
...
async fn reset_password(
    ...
    db.migrate().await?;
    let _crypto =
        crate::services::crypto::CryptoService::initialize(db.pool(), &config.master_key.path)
            .await?;
...
fn print_value(json_output: bool, value: &Value) {
    if json_output {
        println!(
            "{}",
...
fn write_private(path: &PathBuf, contents: &[u8], force: bool) -> Result<()> {
