...
pub async fn login(state: &AppState, request: LoginRequest) -> Result<LoginResult, HttpError> {
...
pub async fn logout(state: &AppState, identity: &AuthIdentity) -> Result<(), HttpError> {
...
pub async fn reauthenticate(
    ...
    let hash = repository::password_hash(state.db.pool(), admin_id)
        .await?
        .ok_or_else(|| {
            HttpError::unauthorized(AUTHENTICATION_INVALID, "The session is invalid.")
        })?;
