...
pub async fn import(
    ...
    for entry in &request.entries {
        match existing.get(&entry.key) {
            ...
            Some(row) if decrypt_row(state, id, row)? == entry.value => {
                unchanged.push(entry.key.clone())
            }
...
async fn decrypt_all(state: &AppState, id: &str) -> Result<Vec<SecretInput>, HttpError> {
