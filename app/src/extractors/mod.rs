...
use crate::{
    constants::{
        ...
        limits::{
            BROWSER_SESSION_IDLE_HOURS, CLI_SESSION_IDLE_DAYS, RECENT_AUTHENTICATION_MINUTES,
        },
    },
    http::HttpError,
...
impl FromRequestParts<AppState> for AuthIdentity {
    ...
    async fn from_request_parts(
        ...
        if let Some(session) = session {
            ...
            let recent: String =
                sqlx::query_scalar("SELECT recent_auth_at FROM sessions WHERE id = ?")
...
pub fn require_mutation(identity: &AuthIdentity, headers: &HeaderMap) -> Result<(), HttpError> {
