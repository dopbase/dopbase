...
impl CryptoService {
    pub async fn initialize(pool: &SqlitePool, path: &Path) -> Result<Self> {
    ...
    pub fn decrypt(
        ...
        let aad = aad(environment_id, key, version);
        let data_key = Zeroizing::new(self.decrypt_master(
    ...
    fn encrypt_master(&self, value: &[u8], aad: &[u8]) -> Result<(Vec<u8>, Vec<u8>)> {
    ...
    fn decrypt_master(&self, ciphertext: &[u8], nonce: &[u8], aad: &[u8]) -> Result<Vec<u8>> {
        if nonce.len() != 24 {
            bail!("invalid nonce");
        }
        let cipher = XChaCha20Poly1305::new_from_slice(&self.master_key).expect("32-byte key");
...
fn aad(environment_id: &str, key: &str, version: i64) -> Vec<u8> {
